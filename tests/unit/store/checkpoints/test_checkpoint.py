...
from openprogram.store.snapshot.checkpoint import CheckpointStore, journal, capture
...
def test_snapshot_accepts_stable_descriptor_metadata_representation(
    ...
    original_fstat = capture.os.fstat
    ...
    monkeypatch.setattr(capture.os, "fstat", descriptor_stat)
    store = CheckpointStore(session_dir)
...
def test_snapshot_rejects_metadata_change_during_read(
    ...
    observed = target.stat()
    original_fstat = capture.os.fstat
    original_lstat = capture.os.lstat
    reads = 0
    ...
    monkeypatch.setattr(capture.os, "fstat", descriptor_stat)
    monkeypatch.setattr(capture.os, "lstat", path_stat)
    store = CheckpointStore(session_dir)
    with pytest.raises(capture.MutationJournalError, match="changed while reading"):
