...
def test_feature_matrix_published_values_match_canonical_table() -> None:
    ...
    assert result.openprogram_score == 88.5
    assert result.openprogram_gaps == 63
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda text: text.replace("OpenProgram为88.5分", "OpenProgram为999分", 1),
            "score",
        ),
        (
            lambda text: text.replace(
                "参考列已确认的 63 项", "参考列已确认的 999 项", 1
            ),
            "gaps",
        ),
        (
