...
2. 输出的时候，模型自动知道自己该输出什么。输出的形式在调用点就约定好了——落回哪个变量、是几个类别里选一个还是一段生成的文本，模型按约定给出结果，代码直接拿到就能用，不再需要专门写一段后处理去解析和分发。这就是我们开源的 [OpenProgram](https://github.com/Fzkuji/OpenProgram) 的做法。整个框架基于 Python 构建：agent 用原生 Python 函数来写，流程控制就是 Python 的 `if`/`for`，生态里的任何库都能直接 import 进函数体，载体是一个装饰器：`@agentic_function`。这篇文章讲它是怎么工作的。
