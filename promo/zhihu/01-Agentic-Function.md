...
我们开源的[OpenProgram](https://github.com/Fzkuji/OpenProgram)就是按这个想法设计的。整个框架基于Python构建：agent用原生Python函数来写，流程控制就是Python的`if`/`for`，生态里的任何库都能直接import进函数体。实现的载体是一个装饰器：`@agentic_function`。把它加在一个函数上，这个函数就成了一个agent，上下文的自动拼接和输出的自动约定都由它完成。这篇文章讲它是怎么工作的。
