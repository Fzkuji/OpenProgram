...
我们开源的[OpenProgram](https://github.com/Fzkuji/OpenProgram)就是按这个想法设计的，核心是一个装饰器：`@agentic_function`。它把agent需要的东西注入到一个Python函数里：docstring作为system prompt，参数作为输入，返回值作为约定好的输出，同时把一个runtime递进函数体，函数体内随时可以通过它发起模型调用，上下文的自动拼接和输出的自动约定都在这条通路上完成。这篇文章讲它是怎么工作的。
