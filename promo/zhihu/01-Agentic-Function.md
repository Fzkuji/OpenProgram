...
我们开源的[OpenProgram](https://github.com/Fzkuji/OpenProgram)就是按这个想法设计的，核心是一个装饰器：`@agentic_function`。加上这个装饰器后，函数在被调用时会收到一个`runtime`参数，函数体内通过`runtime.exec(...)`就能发起模型调用。这些模型调用的system prompt取自函数的docstring，上下文由框架自动拼接（函数的参数、之前的调用历史都在里面），输出按调用点写好的约定解析后落回变量。这篇文章讲它是怎么工作的。
