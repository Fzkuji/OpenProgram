...
于是我们想：能不能让大模型直接活在程序里？模型在做推理的时候，自动就知道自己该干什么、处在什么背景环境下、这个软件运行到了哪一步——对代码的运行状态有直接的认知，而不是每次都靠人把上下文喂给它。做法是把模型调用直接接进函数：函数体里既有普通语句也有模型调用，输入由代码拼出来，输出落回变量接着被代码处理——模型看到的就是程序此刻的真实状态，代码拿到的就是模型此刻的判断。这就是我们开源的 [OpenProgram](https://github.com/Fzkuji/OpenProgram) 的做法。整个框架基于 Python 构建：agent 用原生 Python 函数来写，流程控制就是 Python 的 `if`/`for`，生态里的任何库都能直接 import 进函数体，载体是一个装饰器：`@agentic_function`。这篇文章讲它是怎么工作的。
