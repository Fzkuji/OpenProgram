...
我们开源的[OpenProgram](https://github.com/Fzkuji/OpenProgram)就是按这个想法设计的，核心是一个装饰器：`@agentic_function`。把它加在一个普通Python函数上，这个函数就成了一个agent：函数的docstring就是这个agent的system prompt，参数就是它的输入，返回值就是它约定好的输出。上下文的自动拼接和输出的自动约定，全由这个装饰器完成，函数本身还是原生Python，照常写、照常调。这篇文章讲它是怎么工作的。
