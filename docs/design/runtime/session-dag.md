...
正确:顶层多轮是**同一个根下的兄弟**,靠 seq 排序 + render_context 全可见,**不需要串链**。
...
所有节点只有三种 role：user、llm、code。只有一种边：called_by。ROOT 是唯一的根。

**函数调用节点原则**：调用了什么函数,那个函数就是 DAG 里唯一的节点。绝对不要 anchor、placeholder 或任何辅助节点(持久化/实时/刷新三条数据流见 §六)。

### 分支(fork)

分支 = 同一个位置的另一种可能(平行世界)。**分支节点的 `called_by` 跟被替换的节点完全一样。**

| 场景 | 被替换节点 | 分支节点 | called_by |
|---|---|---|---|
| 用户重发消息 | user2 (called_by=ROOT) | user2' (called_by=ROOT) | ROOT |
| LLM 重试 | llm1 (called_by=user1) | llm1' (called_by=user1) | user1 |
| 工具重试 | code search (called_by=llm1) | code search' (called_by=llm1) | llm1 |

不需要特殊处理——分支节点就是一个普通节点,跟被替换的节点共享同一个 `called_by`。区分哪个是当前活跃的靠 HEAD 指针。在 DAG 图中,分支用虚线连到同级兄弟,往右偏移到独立列。

### Viewport 布局规则

DAG viewport（右侧面板的小地图）按 tree-indent 方式渲染节点。核心规则：

1. **tier（水平列位置）按 role 固定**，不递归走 called_by 链：
   - ROOT: tier=0
   - user: tier=1
...
- **持久化态(权威)**:`@agentic_function` 装饰器在执行返回时把这次调用写成一个 code 节点(`openprogram/agentic_programming/function.py`),其 `called_by` 指向真实调用者。子节点(函数内调的 LLM / 子函数)`called_by` 指向它。这棵子树是唯一真相源。
...
裁定:**SessionStore 的 code 子树是唯一真相源;实时 WS 帧和刷新加载都是它的投影。** 三者对同一次调用必须产出一致的视图(一张卡片、一个方形节点、called_by 一致)。

### called_by 由调用方决定(两种 code 节点)

| 触发方式 | code 节点 called_by | 怎么定 |
|---|---|---|
...
实现上,装饰器读 `_call_id` ContextVar 决定 `called_by`;调用入口(`runtime_attach.py` 的 wrapper)在执行前 set 这个 ContextVar 为真实调用者(用户手动调 = `"ROOT"`,LLM 调 = LLM reply id),执行后 reset。
