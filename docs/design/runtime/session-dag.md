...
**决策 4:聊天换 compute_reads 前,render 的缺口(经核实,只 1 项是真 blocker)。**
原以为 5 项,核代码后:
...
- (e) 压缩/摘要节点 → **不是 blocker,推迟**:compute_reads 不产生摘要 id,且 LLM 摘要现在是 `None`(engine.py:623 "phase5 再加")。重新启用压缩时再在 render 外层 prepend 活跃摘要(`AssistantMessage("[Summary]…")`),不给 DAG 加第4种 role。
...
- **"函数内别调错工具"(如 wiki_agent 自递归)与本决策解耦**——用**处境引导 + 递归深度上限兜底**解(模型据处境自主不调 + 同名超 5 层抛 `RecursionError`),**不靠改 system 的工具列表**(那会破坏统一前缀)。关键一致点:处境提示放在 **user turn 开头**(逐函数变化的内容),system 前缀保持恒定,正好符合本决策。详见 `runtime/execution/agentic-self-recursion.md`。(旧版用 deny 屏蔽工具,已废。)
...
| 4 | 聊天上下文换 compute_reads(flag-gated,default OFF) | 依赖 3 | ✅ 已做 — `context.render=dag` 开启 |
