...
- [`history-node-model.md`](history-node-model.md) — **权威**:历史记录数据模型(一整张图 / 三种节点 user·llm·code / called_by 边 / compute_reads 上下文检索)+ 两套调用路径合并的实现设计
- [`llm-call-unification.md`](llm-call-unification.md) — 调用流程骨架(turn / loop,跟节点模型正交)
- [`agent-worktree.md`](agent-worktree.md)
- [`async-task-lifecycle.md`](async-task-lifecycle.md)
- [`dag-node-model.md`](dag-node-model.md) — 旧节点模型;数据语义已由 `history-node-model.md` 取代,本文仅保留 graph_layout(lane/depth)相关参考
