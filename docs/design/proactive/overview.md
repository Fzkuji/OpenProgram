...
```
①  模型说："我要调 bash 工具，命令是 rm -rf build"
        │
        ▼
②  框架在工具真正执行前，先发出一条"事件"：
        { 发生了什么: "工具即将执行",
        ...
        │
        ▼
③  这条事件被送到所有"订阅了它"的规则面前
        │
        ▼
④  其中一条规则（DangerousCommandGuard）醒来，看了一眼命令：
        "rm -rf ... 这是危险删除" → 决定：拦下来，问用户
        │
        ▼
⑤  框架照做：命令不执行，弹出"确认要删 build 吗？"给用户
```
...
```
空状态  →[事件:工具失败]→  {失败:1}  →[事件:工具失败]→  {失败:2}  →[事件:工具成功]→  {失败:0}
```
...
```
   事件流（流水账，只往后记）
        │
        ├──> fold 累加 ──> State（当前状况：改了哪些文件、某工具失败几次……）
        │                        │
        ▼                        │ 规则读它
   新事件来了                     ▼
        │                  ┌─────────────┐
        └─────────────────>│   规则们     │  每条规则：盯某类事件 + 读 State + 决定出手
                           │  Policy      │
                           └──────┬───────┘
                                  │ 返回一个动作
                                  ▼
                          挡路的：拦/放/问        旁观的：提醒/注入/后台准备
                                  │                      │
                                  ▼                      ▼
                           框架去落地（弹窗、拦截、注入……）
```
