...
> 代码: `context/engine.py`、`context/budget.py`、`context/microcompact.py`、`context/tool_aging/`、`context/summarize.py`、`context/persistence.py`、`context/nodes.py`（`render_context`）。
...
- **函数调用级**（agentic function 内部）：模型只看到自己和父辈的调用（纵向），同辈函数的内部不可见（通过 `render_context` 的 `expose` 机制隐藏）
...
在 `render_context` 返回节点列表之后、`render_dag_messages` 渲染之前，插入一个精简步骤：
...
```
DAG 节点 ─┬─ render_context（expose 过滤，render_range 裁剪）← 已有
           │
...
节点级控制在 `render_context` **之后**、`render_dag_messages` **之前**插入：

1. `render_context` 先按 expose/render_range 做粗筛（这步决定哪些节点"有资格"出现）
...
| 渲染入口 | `render_context(frame_entry_seq=-1)` → 全部节点 | `render_context(frame_entry_seq=N)` → N 之后的节点 + N 之前受 callers_cap 限制的 |
| 压缩时机 | dispatcher 每轮调用前 | runtime.exec 每次调 LLM 前（§3 的改进） |

### 6.2 统一精简策略

不管在哪个场景，`apply_visibility` 的输入都是 `render_context` 的输出（一个节点 id 列表）。精简逻辑相同：

1. 对输入列表中的节点计算层级
2. 按预算分配 visibility
3. 返回 `(node_id, visibility)` 列表

差异由 `render_context` 处理（它已经根据 `frame_entry_seq` 和 `render_range` 做了场景区分）。`apply_visibility` 不需要知道自己在会话级还是函数调用级。
