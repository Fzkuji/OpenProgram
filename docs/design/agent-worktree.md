...
OpenProgram 自己有 `~/.agentic/sessions-git/<sid>/`（每个 session 一个 git repo），
存对话内存的 history / context / workdir。**绝对不能把 agent worktree
落地在这个目录树里**：

- worktree_path 必须不在任何 `~/.agentic/sessions-git/*` 之下（D14 校验）。
...
不提供"discard 前自动备份"。讨论过把丢弃的内容打 tar 塞 `~/.agentic/discarded/`，
...
- `worktree_in_sessions_dir`: source_repo 落在 sessions-git 树里（D4 隔离违例）
...
- `source_repo` 在 sessions-git 树里 → `worktree_in_sessions_dir` 拒绝（D4）。
...
| **D4 隔离** | worktree_path 不在 sessions-git 树里：默认 `~/.agentic/worktrees/<id>-<slug>/`（独立目录，跟 source_repo 平级） |
...
| 3 | 新建 `openprogram/worktree/_paths.py` | worktree path 策略：`~/.agentic/worktrees/<id>-<slug>/`；隔离校验（D4）|
...
1. **worktree_path 永远不在 `~/.agentic/sessions-git/` 子树里**
...
- **discard 前的自动备份**：D9 提到的 `~/.agentic/discarded/` 打包，留作未来加强。
