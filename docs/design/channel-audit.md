...
来源：`references/hermes-agent/gateway/`。Hermes 是 chat-bot 专门项目，对接 14+ 平台（Discord / Slack / Telegram / WhatsApp / Signal / IRC / Mattermost / Google Chat / Matrix / Simplex / LINE / Email / Home Assistant / ntfy）。

### 2.1 BasePlatformAdapter ABC
...
| 方面 | OpenProgram | Hermes |
|---|---|---|
| Base abstract method 数 | 1 (`run`) | 5+ (`send/edit/draft/typing/handoff`) |
| 中性消息结构 | 无 (平台原生 obj) | `MessageEvent` + `SessionSource` dataclass |
| Send 返回值 | bool | `SendResult` (含 message_id + retryable) |
| Dispatch signature | 同步 → str | 异步 → 流式事件 |
| Session 隔离维度 | 1 维 (peer_id) | 2 维 (chat + user/thread) |
| Edit/Reaction 接口 | 无 | 内建 |
| 进度流 | 无 | edit_message 节流 + overflow 自动分组 |
| Debounce 合并 | 无 | 0.35s delay + 1s hard cap |
| 快速命令绕路 | 无 | `/stop /approve` 不进队列 |
| Retryable 信号 | 无 | `SendResult.retryable` 区分 transient/permanent |
| Attachment 缓存 | 无 | UUID-前缀 + 24h 清理 |
| 出站 API | `outbound.send` 走 raw HTTP | `DeliveryRouter` 走 adapter |
| 反向依赖 | outbound → wechat 私有函数 | 无 |
| Chunking 实现 | 5 份重复 | 平台内统一 (`truncate_message`) |
| MAX_CHARS 定义 | 5 个文件 | adapter 内一份 |
| Platform 注册 | 硬编码 dict | hybrid (built-in + registry) |

### 3.2 直接后果

| 我们想做的 feature | 现在改的范围 | hermes 改的范围 |
|---|---|---|
| Progress streaming | 改 base + 4 adapter + outbound + _conversation = 6 处 | dispatcher 调 adapter.edit_message 一处 |
| Reaction approval | 4 adapter 各加 listener + adapter ↔ approval bridge | 加一个 `/approve` slash handler |
| Edit message | base + 4 adapter + outbound = 6 处 | adapter.edit_message 一处 (已有) |
| 加新 platform (whatsapp) | adapter + outbound._send_xx + chunk + bindings + accounts | adapter 一处 + registry.register |
...
4. `outbound.send` 是删了改走 adapter，还是保留作为"快路径" fallback？
5. WeChat 不支持 edit 怎么处理——`edit_message` raise vs 假实现 vs base 接口本身做成 optional？
6. session_scope 要不要扩成 hermes 的二维（chat × user × thread）？
