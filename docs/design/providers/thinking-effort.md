...
| 各 provider probe_thinking.py | ✅ 已落地 | 每个 provider 文件夹里有自己的探测脚本，Fetch 时自动调用 |
| Fetch 时自动探测 reasoning | ✅ 已落地 | `fetchers/__init__.py` 在 enrichment 步骤自动调 `probe_thinking.probe()` |
| thinking.json 自动更新 | ✅ 已落地 | `probe_thinking.py --update` 从 API 写入 model_overrides |
| 第 3 层试探降级 | 待实现 | probe_thinking_levels 函数 + 缓存 |

## 14. 各 provider 的探测文件

每个 provider 文件夹里有 `probe_thinking.py`，Fetch 时自动被 `fetchers/__init__.py` 调用，不需要用户手动运行：

| Provider | 文件 | 探测方式 |
|---|---|---|
| anthropic | `anthropic/probe_thinking.py` | `/v1/models/{id}` capabilities（精确） |
...
| openai_codex | `openai_codex/probe_thinking.py` | DeepSeek model id 推断 |
