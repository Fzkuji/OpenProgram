...
Opening a conversation activates its existing tab. If it is not open, the current session tab is reused, or a new session tab opens beside a webpage. Window history restores sessions in visit order while preserving draft input and background title updates. Existing per-tab session history is still saved with the tab and remains available after reload; deleted sessions are removed from history.
...
PDF files open in a read-only viewer with page navigation, zoom, text search and original-file download. The PDF decoder is included locally and supports the bundled desktop runtime; opening a PDF does not upload it to an external viewer. Preview input is limited to 64 MiB.
