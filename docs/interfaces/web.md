...
To branch an Agent from a saved point, enter the new instructions in **Branch with new instructions**, then prepare, check, approve, and publish the revision. **Create branch** selects a separate paused child; **Continue** starts it with the published instructions. Its message and checkpoint state are independent of the original execution. The editor supports additional Agent instructions, rather than arbitrary source-code or model-setting edits.

Retry creates a separate paused Agent execution at the saved point, keeping the original revision. Select **Continue** to resume it. Instruction-only branches preserve the program and runtime contracts, so they do not require a second approver; changes to program, tool, model, or output contracts retain independent approval. Editing unpublished instructions creates a new draft version and invalidates its previous checks and approval.
