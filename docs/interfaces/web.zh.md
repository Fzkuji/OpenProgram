...
选择 Agent 后查看进展并使用已有执行控制。暂停、继续、单步和重试仅在执行确实支持时提供。内部 ID、修订信息、保存点和原始事件放在“技术详情”中。结果待确认表示执行受阻，不是持续生成。没有活动 attempt 时，输入框允许发送新消息，刷新后不会恢复取消按钮；活动面板继续保留待确认结果及其操作限制。快照读取成功也不代表 Agent 正在运行。
