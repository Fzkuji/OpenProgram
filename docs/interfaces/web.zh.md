...
要从 Agent 的保存点创建分支，在“按新指令创建分支”中输入指令，然后准备、检查、批准并发布修订。创建分支后选中独立的暂停子执行，点击继续后按已发布指令执行；原执行的消息和检查点不变。此编辑器支持补充 Agent 指令，不将任意源码或模型配置编辑伪装成已支持的功能。

重试在保存点创建独立、暂停的 Agent 执行，保留原修订；点击继续后恢复。只补充指令的分支保留程序和运行契约，不要求第二位审批者；修改程序、工具、模型或输出契约仍要求独立审批。修改未发布的指令会生成新草稿版本，原校验和审批不再有效。
