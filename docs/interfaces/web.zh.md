...
打开会话会激活已有会话标签；如果尚未打开，则复用当前会话标签，或在网页旁新建会话标签。窗口历史按访问顺序恢复会话，草稿输入和后台更新的标题会保留。每个标签原有的会话历史仍随标签保存，重新加载后可继续使用；已删除会话会从历史中移除。

在 Files 或文件标签页中，后退和前进会依次返回当前项目中浏览过的文件夹与文件。返回 Files 时恢复文件夹展开状态和滚动位置；后退后打开新位置会替换原有前进记录。当前窗口最多保留 100 个位置，与文档版本 History 分开。目录列表快照失效时，重新读取已加载范围期间保留已有文件行。

PDF 文件在只读查看器中打开，支持翻页、缩放、文本搜索和下载原文件。PDF 解码器随程序本地安装，兼容 App 内置运行时；打开 PDF 不会将文件上传至外部查看器。预览文件大小上限为 64 MiB。
