...
点击右侧栏的 **资源**，打开 **会话资源**。通过侧栏折叠按钮收起或展开面板。可按资源名称、目标、类型或会话搜索，并展开或折叠会话分组。面板列出网页、有归属记录的文件视图、终端视图、活动后台进程和集成上报的资源；没有会话归属的手动视图单独分组。

Agent 网页默认不占顶部标签，固定或显式分屏时保留在顶部。点击网页或文件可打开原有视图；网页支持固定和关闭。折叠面板或分组不会停止资源。选择资源后，资源面板继续显示在视图右侧。
