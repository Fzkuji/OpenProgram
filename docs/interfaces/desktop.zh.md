...
若 Agent 在你停留在聊天时打开页面，桌面端会把该实时 WebTab 显示为角落小窗。小窗可以展开为「左聊天右网页」分屏、接管中间栏，或关闭预览且不销毁标签。关闭预览只是隐藏小窗，页面仍可从左上角的 **会话资源** 中访问。网页版（普通浏览器里的 UI，不是桌面 App）没有原生 BrowserView，同一预览降级为 iframe 或「在新标签打开」。
...
点击顶部左侧、窗口控制按钮旁的下拉按钮，打开 **会话资源**。可按资源名称、目标、类型或会话搜索，并展开或折叠会话分组。弹窗列出网页、有归属记录的文件视图、终端视图、活动后台进程和集成上报的资源；没有会话归属的手动视图单独分组。

Agent 网页默认不占顶部标签，固定或显式分屏时保留在顶部。点击网页或文件可打开原有视图；网页支持固定和关闭。关闭弹窗或折叠分组不会停止资源。右侧栏不再提供 Pages 入口。

Docker 与 SSH 前台后端调用记录使用期间的资源。后台进程保留现有状态与输出。GUI Harness 在调用期间记录配置的桌面或 VM 关联。点击非网页资源可查看目标和状态，后台进程还能查看输出。这些记录表示会话使用情况，不提供容器或 VM 删除操作。

其他集成可以在可信运行时会话内使用 `openprogram.session_resources.resource_use(kind, title, target)` 上报任意资源类型。上下文自动记录真实会话，退出后释放使用记录。系统不会从任意 shell 命令文本推断资源；资源 URL 会去掉凭据、查询参数与片段。
