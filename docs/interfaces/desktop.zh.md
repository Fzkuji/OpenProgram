...
网页正在回来时，资源行留在原分支，显示 **正在恢复网页…**。恢复失败时同一行显示 **网页恢复失败**。新的活页面之后需要重新连接时，显示 **需要重新连接**。这些行不会进入通用的不可用分组。只有你明确关闭的网页出现在 **已关闭的网页**。恢复网页不会启动或继续 Agent。空闲页面仍可直接操作。新任务仍按常规重新观察页面并检查权限。上一进程中的表单或其他页内编辑不会恢复。Desktop `webtabs` 分区里已经保存的 Cookie 与站点存储可以在页面重新加载时生效。以上是已批准的产品行为；测试与默认 App 验收仍待完成。
