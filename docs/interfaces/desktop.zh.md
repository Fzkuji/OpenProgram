...
点击右侧栏的 **资源**，打开 **会话资源**。通过侧栏折叠按钮收起或展开面板。面板仅显示当前会话所属的资源，可按名称、目标或类型搜索；切换会话会清空此前的搜索和详情。面板列出网页、有归属记录的文件视图、终端视图、活动后台进程和集成上报的资源；没有归属记录的视图和新建草稿会话不显示会话资源；打开已有归属的资源时保留其会话上下文。

有会话归属的 Agent 网页默认不占顶部标签，固定或显式分屏时保留在顶部；没有归属记录的旧网页保留在顶部。点击网页或文件可打开原有视图；网页支持固定和关闭。折叠面板或分组不会停止资源。选择资源后，资源面板继续显示在视图右侧。
