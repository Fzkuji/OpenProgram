...
默认入口页也属于访问历史。标签栏的后退／前进按当前窗口的实际跳转顺序，恢复入口页、内置页面、侧栏页面、应用、会话和文件浏览位置；打开已经存在的目的地也能返回入口页。后退后选择新目的地会清除原来的前进记录。关闭或删除的目的地会被跳过，窗口访问历史只在当前窗口运行期间保留。网页内部的 URL 前进后退仍由浏览器控件处理。
...
会话历史先加载最近一页。点击消息列表上方的“加载更早消息”读取旧记录，不会覆盖正在输出的新内容。历史分页保持在同一会话分支内。重新连接会加载最近一页，不改变已保存的历史或模型上下文。
