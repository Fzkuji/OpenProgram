...
- A Web session can be resumed in the terminal via `/resume`, or targeted directly with `openprogram --resume <session-id>`.
...
| [Terminal TUI](tui.md) | `openprogram` (no arguments) | Full chat without leaving the terminal: slash commands, permission-profile switching, scrollable history |
