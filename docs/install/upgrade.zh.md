...
Running 使用同一状态卡。默认摘要显示尝试次数与当前阶段，展开后查看版本、证据和操作。展开后的字段统一按标签和值对齐，完整标识符、变更文件和验证时间集中在详情与证据区。视图直接复用执行过程的折叠组件和消息操作组件，不覆盖它们的样式。目标版本与最近已验证运行版本分开显示：没有运行证据时
保持“未知”，先前验证不代表当前仍然在线。展开“详情与证据”可查看 assertions，
并以纯文本加载需要鉴权的证据。状态无法读取时保留最近快照，明确标注可能过时和
最近同步时间，然后自动重试。

消息操作按钮打开与“修改后重新运行”相同的参数编辑器。“编辑重试请求”要求输入新候选提交的完整 SHA；取消和停止迭代也使用该编辑器。“准备请求”只向原会话未发送草稿追加内容，保留原有草稿。需要在原会话发送草稿，
