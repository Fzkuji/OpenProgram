...
Running 使用同一状态卡。默认摘要显示尝试次数与当前阶段，展开后查看版本、证据和操作。展开后的字段统一按标签和值对齐，完整标识符、变更文件和验证时间集中在详情与证据区。卡片与聊天正文共用字体和字号变量，按钮与输入框复用公共组件。目标版本与最近已验证运行版本分开显示：没有运行证据时
保持“未知”，先前验证不代表当前仍然在线。展开“详情与证据”可查看 assertions，
并以纯文本加载需要鉴权的证据。状态无法读取时保留最近快照，明确标注可能过时和
最近同步时间，然后自动重试。

“更新操作”中的取消、停止迭代和重试按钮只向原会话的未发送草稿追加请求，
