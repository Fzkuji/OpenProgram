...
创建表单只填写任务，不提供高级参数或角色、预算编辑入口。新 Goal 沿用已配置模型，由一次无工具模型请求根据任务难度选择模型支持的推理强度；响应无效或选择失败时沿用默认值。恢复任务沿用保存的设置，超时和预算保留现有默认值。Python 和 CLI 调用仍支持显式覆盖。
...
目标暂停或等待时，Python 和 CLI 调用可修改工作和判定角色的 provider、模型、推理强度及超时。Provider 是已配置的认证路由，不是凭证。保存不改变目标、证据、问题与累计用量；恢复时验证模型。模型不可用时，可修正选择后再次恢复。执行中的目标需要先暂停才能修改角色。

TUI 使用相同设置，不需要打开浏览器。

使用 `openprogram --resume <session-id>` 重新打开已有会话及其保存的 Goal。
发行包包含终端界面；仍需 Node.js，但不需要源码目录或运行 npm 安装。
...
composer 中的 Goal 状态条会打开详情弹窗。弹窗显示目标、状态、checklist、资源用量、最近判定原因和全部待答问题，并支持编辑、暂停、继续、逐项回答和终止。
