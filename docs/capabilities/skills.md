...
2. 所有 skill 的 name、一行 description 和 `SKILL.md` 的绝对路径渲染成一个 `<available_skills>` 块，追加到 system prompt。
3. 模型判断当前任务是否匹配某个 skill 的 description；匹配时用 `read` 工具读取 `SKILL.md` 全文。全文不会自动注入。

skill 本身不执行任何东西。`SKILL.md` 旁边可以放脚本、参考文件、数据，由现有的 `bash` / `execute_code` 工具运行。
...
默认按顺序探测两个位置，同名时先见者胜，因此**用户 skill 覆盖仓库 skill**：

1. `~/.openprogram/skills/`（用户级）
2. `<OpenProgram 仓库>/skills/`（项目级）

插件也可以贡献 skill（见 [Plugins](plugins.md)）；从远端安装的 skill 落在 remote-cache 源里。

## 管理命令

```bash
openprogram skills list       # 按来源（project / user / remote-cache）列出已发现的 skill
...
openprogram skills update     # 对比本地 SKILL.md 哈希与上游，重拉过期的
