...
网页标签与其预览不得同时显示。真实网页在自己的标签中保持可用，不会变暗或被遮罩阻挡。工具栏显示当前操作者，并提供“接管”“恢复”“显示操作”和“操作记录”。聊天预览使用单行紧凑控件栏；空闲时不提供可点的接管。默认小窗为 400×255。拖动标题栏移动，或从任一边/角调整大小；画面比例保持联动，底层页面不被缩放。操作位置提示短暂显示，不拦截输入；记录显示操作类型和结果，不保存网页中输入的完整文字。
