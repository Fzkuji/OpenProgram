...
单击资源在聊天中查看它的图像预览。“跟随”显示该分支最近实际操作的网页；手动选择资源后，即使 Agent 操作其他网页，也保持当前选择，且不抢占标签焦点。“隐藏”停止预览截图并保留网页，“展开”放大图像预览，“打开页面”把原来的 Page 显示为普通顶部标签并隐藏聊天预览。返回会话后，除非已经隐藏，否则恢复预览。这些选择按会话和分支保存。

网页标签与其预览不得同时显示。真实网页在自己的标签中保持可用，不会变暗或被遮罩阻挡。工具栏显示当前操作者，并提供“接管”“恢复”“显示操作”和“操作记录”。聊天预览使用紧凑标题栏和带文字的操作栏；空闲时不提供可点的接管。操作位置提示短暂显示，不拦截输入；记录显示操作类型和结果，不保存网页中输入的完整文字。
...
本地桌面任务会在规划前检查 macOS 权限。缺少权限时，对话保留原任务，并为刚结束且当前可见的运行打开系统授权窗口。完成授权后回到对话，点击「继续任务」；程序会重新检查权限，再使用保存的参数执行同一项 GUI 任务。查看历史记录不会反复弹出系统设置。浏览器和虚拟机任务不会申请本地桌面权限。
