...
单击资源在聊天中查看它的图像预览。“在会话中预览”是针对同一现有 Page 的显式控件：不增加顶部标签，进入手动查看；若当前正在查看网页，则回到所属会话，使聊天预览可见。“跟随”显示该分支最近实际操作的网页；手动选择资源后，即使 Agent 操作其他网页，也保持当前选择，且不抢占标签焦点。“隐藏”停止预览截图并保留网页，“展开”放大图像预览，“打开页面”把原来的 Page 显示为普通顶部标签并隐藏聊天预览。返回会话后，除非已经隐藏，否则恢复预览。这些选择按会话和分支保存。

网页标签与其预览不得同时显示。真实网页在自己的标签中保持可用，不会变暗或被遮罩阻挡。工具栏显示当前操作者，并提供“接管”“恢复”“显示操作”和“操作记录”。聊天预览使用单行紧凑控件栏；空闲时不提供可点的接管。操作位置提示短暂显示，不拦截输入；记录显示操作类型和结果，不保存网页中输入的完整文字。

在真实网页中点击、输入、滚动或导航会请求让出控制：停止接纳后续 Agent 输入，并请求暂停执行。仅聚焦页面或查看预览不会暂停。已经发送的动作仍可能完成。“正在让出”表示停止尚未确认；“已暂停”需要执行确认及状态核对。“停止未确认”表示 App 尚未确认停止，真实网页仍然可以正常使用。恢复需要明确操作，并重新观察页面、检查当前权限。
...
本机桌面任务缺少系统权限时，在操作前暂停。OpenProgram 直接请求缺少的系统权限，必要时打开对应的系统设置页面。会话只显示简短等待状态及“打开系统设置”文字入口，不显示权限表单或继续按钮。

后台保存等待中的任务，并检查权限是否已对实际执行进程生效。权限可用后，同一个任务继续一次。关闭页面不会停止检查；如果需要重启应用，启动后会再次检查保存的任务。取消任务后不会恢复。检查失败时保留等待状态，不会当作已经授权。查看旧会话本身不会启动操作或重新弹出系统授权。系统授权不会替代工具操作审批。
