...
单击资源在聊天中查看它的图像预览。“在会话中预览”是针对同一现有 Page 的显式控件：不增加顶部标签，进入固定预览；若当前正在查看网页，则回到所属会话，使聊天预览可见。聊天预览标题栏提供持续的“固定预览 / 取消固定预览”：固定后保持当前页面；取消固定后恢复自动显示该分支最近实际操作的网页。选择资源后，即使 Agent 操作其他网页，也保持当前选择，且不抢占标签焦点。该固定控件只在聊天预览上，会话资源没有固定按钮。“隐藏”停止预览截图并保留网页，“展开”放大图像预览且不改变自动或固定预览，“打开页面”把原来的 Page 显示为普通顶部标签并隐藏聊天预览。返回会话后，除非已经隐藏，否则恢复预览。这些选择按会话和分支保存。

网页标签与其预览不得同时显示。真实网页在自己的标签中保持可用，不会变暗或被遮罩阻挡。工具栏显示当前操作者，并提供“接管”“恢复”“显示操作”和“操作记录”。聊天预览使用单行紧凑控件栏；空闲时不提供可点的接管。默认小窗为 300×198.75.拖动标题栏移动，或从任一边/角调整大小；画面比例保持联动，底层页面不被缩放。操作位置提示短暂显示，不拦截输入；记录显示操作类型和结果，不保存网页中输入的完整文字。
