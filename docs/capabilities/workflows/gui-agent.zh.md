...
单击资源查看它的图像预览。“跟随当前分支”显示该分支最近实际操作的网页；手动选择资源后，即使 Agent 操作其他网页，也保持当前选择。“隐藏”停止预览截图并保留网页，“展开”放大图像预览，“在标签页打开”打开原来的 Page，供人直接使用。这些选择按会话和分支保存。

真实网页可以与预览同时使用，不会变暗或被遮罩阻挡。工具栏显示当前操作者，并提供“暂停 Agent”“恢复 Agent”“显示操作”和“操作记录”。操作位置提示短暂显示，不拦截输入；记录显示操作类型和结果，不保存网页中输入的完整文字。
