...
若 App 或 worker 重启，你尚未关闭的网页在后台恢复，不额外增加顶部标签。保留的 tab、会话与分支以及最后确认的标题和 URL 保持不变。应用为该保留 tab 创建新的活页面。先前的活句柄无效。明确关闭的网页留在“已关闭的网页”。已隐藏的预览保持隐藏，直到你选择该资源或使用“在会话中预览”。正在恢复、恢复失败或需要重新连接的行留在原分支，分别显示“正在恢复网页…”、“网页恢复失败”或“需要重新连接”，不进入通用不可用分组。恢复网页不要求“让 Agent 继续”。空闲页面仍显示“可直接操作”。新任务动作仍须在新页面上重新观察并检查当前权限。上一进程中的 DOM 与表单值不会恢复。重启恢复是已批准的预期行为；实现验收仍待完成。
