...
| 实体 | `archive_agent` | 把自己创建的 agent 归档：它从 `list_agents` 消失，并拒收后续 `send_message` / `agent(to=)` 投递；`read_conversation` 照读它的历史，`agent(start_from="SID:MSG_ID")` 照 fork。只有创建它的会话能归档，且没有反归档 | 无 |
