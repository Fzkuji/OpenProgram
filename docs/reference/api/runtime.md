...
| `max_iterations` | `int` | `20` | Upper bound on tool-loop iterations (one iteration = one model call plus its tool execution). The effective value is `max(1, min(200, max_iterations))`, where 200 is the hard limit in `agent_loop.py` |
