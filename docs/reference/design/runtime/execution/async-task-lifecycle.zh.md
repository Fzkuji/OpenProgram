...
`tasks.json`，结构 `{task_id: TaskRow}`。每个 task 状态转移调一次
...
- `task_created`、`task_status`（queued / running / completed / cancelled / errored）、`task_progress`（可选，未来可加 token 进度）。
- 复用现有 `running_task` 广播并加 `task_id` 字段（UI 兼容）。
...
- `task_output(task_id, block=True, timeout=30000)` → 等待(timeout 毫秒,上限 600000,对齐 Claude Code 的 TaskOutput 参数形状)直到完成 / cancelled / 超时,返回回复文本+终态;`block=false` 立即窥探当前状态。`list_tasks()` 列出本会话的后台任务(id、状态、主题),LLM 用它掌握并行工作的全景。
...
| **D4 持久化** | 同 A |
| **D5 cancel** | LLM 调 `task_stop(id)` 或用户 UI cancel；两条路径一样（都进 runner.cancel） |
| **D6 session 绑定** | 同 A |
...
| **D10 agent tool** | `spawn_task` 返回 `task_id` 给 LLM；后续 `task_output(task_id)` 拿结果 |
...
| **D5 cancel** | `task_stop(id)` 单个；plan 整体撤销时 plan agent 自己遍历 cancel 所有 children（也可以加个 `cancel_tasks(parent_msg_id=...)` 批量 API 作为后续扩展） |
...
| **D10 agent tool** | LLM 可以调 `task_stop(id)`；后续 `task_output(id)` 立刻返回 cancelled 终态 |
...
| 5 | `openprogram/agent/task/agent_tools.py` (新建) | `@function` 实现 `spawn_task / task_output / task_stop / await_tasks`，绑定到 toolset (D10) |
