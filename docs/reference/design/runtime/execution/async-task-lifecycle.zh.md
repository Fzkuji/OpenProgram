...
- agent-facing tool `task(prompt, ...)` 是 **async** 的——立刻返回 `task_id`，不阻塞主对话。LLM 拿到 id 后选择继续干别的，或者立刻调 `await_task(task_id)` 取得同步语义（D15）。
...
最常见情况：LLM 调 `task(prompt="探查 X")`，希望阻塞拿到结果。行为与同步工具
一致，但底层走 task entity。
...
| **D5 cancel** | `cancel_task(id)` 单个；plan 整体撤销时 plan agent 自己遍历 cancel 所有 children（也可以加个 `cancel_tasks(parent_msg_id=...)` 批量 API 作为后续扩展） |
