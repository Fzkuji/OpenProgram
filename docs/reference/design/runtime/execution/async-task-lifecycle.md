...
session repo sits a `tasks.json` with structure `{task_id: TaskRow}`. Each task
...
- On spawn, a **placeholder attach card** is written immediately (`function="attach"`, `extra.attach.task_id = <task_id>`, `extra.attach.status = "running"`), content="(running)", with `source_commit_id` left empty.
...
- `task_created`, `task_status` (queued / running / completed / cancelled / errored), `task_progress` (optional; token progress can be added in the future).
- The existing `running_task` broadcast is reused with an added `task_id` field (UI compatibility).
