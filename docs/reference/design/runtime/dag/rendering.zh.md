...
占住，此时它取下一个空行并带上自己的 lane，因为子 agent 胶囊是一颗装着名字的
药丸，两颗并排读不了（第十二节）。跨会话 spawn 落到目标会话时是该会话自己的
对话链（lane 0），不是侧枝（场景 12）。
...
**名字画在药丸内部**。压缩用的数量只有三个字符，挂在胶囊右侧没问题；名字不行。同一轮
spawn 出的两个子 agent 只隔一条 lane，两个挂在右侧的名字会互相印在对方身上、也印在对方
的身体上。所以药丸按自己实测的文字宽度撑开、把名字居中装在里面，超过 180px 截断加省略号
——胶囊是一个字形，不是一段要整条 lane 让路的段落。药丸只朝**右**长，左端帽始终停在普通
胶囊的位置，绝不回头盖住发起它的那个节点。这个宽度也是布局读的同一个数：它据此留出药丸
占的列、把画布按药丸右缘而不是锚点定宽，并让每一条可能终止在胶囊上的边落在药丸边缘而不是
中心，于是没有任何线穿过名字。

**两颗胶囊永不同行**。药丸既然是宽的，上面的行例外就要给它让路：第一颗胶囊保留它调用节点
所在的行，第二颗若会落到同一行则取下一个空行，并带着自己的 lane 一起下移（展开的分支必须
跟着它的头）。规则②依然成立——这是排序，不是预留空行。
