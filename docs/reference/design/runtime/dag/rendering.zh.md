...
**分支按实际占用列紧贴排**：一条分支占用的列 = 起始列到其子树最深一格——包括
子 agent 头旁注文划过的那几列（第十二节，注文墨迹也预留列）；下一条分支从上一
条实际占用的最右列 +1 开始，互不重叠。
...
分支首节点与 spawn 调用节点**同一行**（场景 10）——除非该行已被另一个 spawn 根
占住，此时它取下一个空行并带上自己的 lane，因为子 agent 头的名字是横在旁边的
注文，两条并排会互相穿过（第十二节）。跨会话 spawn 落到目标会话时是该会话自己
的对话链（lane 0），不是侧枝（场景 12）。
...
胶囊：它横向比基准圆宽，但仍以单个格点为中心、按单个格点安放。注文墨迹不是
形状，但布局仍替它预留划过的列、并让两个带注文的头永不同行（第一节），于是
名字永远不会印在别的节点上。
...
（`_spawnHW`），lane 打包为这段延伸预留列，包围盒延到它那里让 fit 永不裁掉名字，
两个带注文的头也永不同行（第一节）。
...
| 第一节 lane / tier / depth 布局 | `dag/layout/geometry.ts::computeGeometry`（按内容打包 lane、前序分行、注文墨迹预留列、头节点行认领）；格点性、无重叠、行认领、预留墨迹均由 `web/scripts/check-dag-subagent.mjs` 真实执行并断言 |
