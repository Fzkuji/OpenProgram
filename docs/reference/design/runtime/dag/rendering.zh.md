...
因为压缩吃的永远是会话开头的前缀——图画的正是这条边。压缩由此成为*会话起点
处的一次分叉*：胶囊是开头轮次的替代版本，与所有 retry 分支共用场景 3 的视觉
语言。折叠时（在携带它的分支上），原文前缀隐藏，胶囊占据主干首位、继承幸存
者的 lane。展开时两个版本并排站立——幽灵前缀在主干上，胶囊与第一个被覆盖轮
同排作它的姐妹，带桥线。除折叠态的 lane 继承外没有任何视图层改接；画的边就
是存的边。
