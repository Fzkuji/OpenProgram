...
点的多条分支按派生顺序依次占下一行。两者之间的虚线桥因此是一段两格长的同行
横线。
...
| 第四节 HEAD 实心、无光环 | `render/nodes.ts` 把 `isHead` 作为 `_buildShapeEl` 的 `solid` 传入；覆盖点从实心里挖出；`right-dock.css` 已无 `.is-head` 光晕；HEAD 指向已归并回复时落到锚上（`pipeline.ts` 经 `threadModel.anchorOf`） |
