...
**胶囊的位置**。折叠时它站在覆盖区间原来的位置——主干最前，因为摘要是下一次
请求携带的第一样东西。展开时区间本身回到屏上，胶囊读作这些轮次收拢*成*的
结果：它接在最后一个幽灵之后、保留尾巴之前——幽灵 → 胶囊 → 尾巴。两种摆位
都是 `fold-summaries.ts` 里仅视图层的改接；存储行在两种状态下都保留真实的
`predecessor`（区间开始的地方）。
...
除摘要自己（它自己的 seq 恰好排在它所指区间的里侧）。扫描限制在 HEAD 链加上
被覆盖轮次的 caller 子树：压缩总结的是一条分支的上下文，seq 恰好落进区间的
死分叉永远不会被折进胶囊。
