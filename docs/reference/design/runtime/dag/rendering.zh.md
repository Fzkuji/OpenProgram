...
能发现的 bug。fit 把平移量吸到整像素，正是为此。

**视角状态跨重绘保持。** 图每次 capture 都重绘，每次都动镜头会在用户读图时把
视野拖走。平移与缩放按会话存在 `store/globals` 里，只有进入另一个会话才重新
fit。改面板尺寸永不重新 fit——用户看图的角度是他自己的。

**HUD。** 右下角、输入框上方，规格同输入框自己的环境 chip：一个 fit 按钮、缩放
百分比、图例弹层。缩放读数由 `canvas.ts` 在每次视角变化时命令式写入——把一次
手势的每个 wheel 事件走 React state 会让整棵树每秒重绘六十次。
...
分支从上一条实际占用的最右列 +1 开始，互不重叠。注文墨迹不预留任何列——子
agent 的名字从相邻 lane 的格子上方飞过，而行认领（第十二节）保证它飞过的格子
都是空的。
...
分支首节点与 spawn 调用节点**同一行**（场景 10）。

两个例外都让位于**已认领的行**。子 agent 头的注文和 fork 根的虚线桥都是横贯整
行的墨迹——前者把名字横过右侧的 lane，后者把线横过左侧的 lane——所以每个头、
每个 fork 根按调用顺序各认领一行，撞上已认领行的取下一个空行并带上自己的
lane（第十二节）。行没被占的头和 fork 保留锚点行，上面两条例外照常成立。跨会
话 spawn 落到目标会话时是该会话自己的对话链（lane 0），不是侧枝（场景 12）。
...
**③ 字形占格，文字是注文**：一个节点占它那个格点；名字与计数用注文灰挂在旁边，
绝不装进按自身文字定尺寸的形状里。跟着标签长的字形，是每个邻居都得拿它量一遍
的字形——这场谈判正是第十二节早先那颗药丸做错的事。唯一的例外是第九节的压缩
胶囊：它横向比基准圆宽，但仍以单个格点为中心、按单个格点安放。注文墨迹不预留
任何列——让名字不印在节点上的是**行的排他性**：子 agent 头和 fork 根这两种横贯
整行的墨迹各自认领独占的行（第一节），注文飞过的格子因此都是空的。
...
| spawn 边（发起节点 → 子 agent 点） | 虚线曲线 `5 4`，从发起轮下缘出发 | 子分支色 | 显示 |
...
◎ 子 agent 头（两个同心圆，另一个 agent 那条链的头，见第十二节）。

**HEAD 画成实心**：字形自身填上分支色，不是套一圈光环或描边。这个尺寸下光环读
起来像旁边多了一个更糊的节点，而且是缩小时第一个丢失的东西。实心 HEAD 上的覆盖
标记是**挖出来的**——别的节点显白点的位置，它显一个背景色的点。
...
| `↗`（右上角） | 跨会话 spawn 的**两侧都标**：目标会话里的分支根（caller 在另一个会话的图里，本会话内挂 ROOT，tooltip "spawn 自 <源会话>"）；源会话里发起 spawn 的那个节点（tooltip "派往 <目标会话>"——否则这轮派了活在自己图里毫无痕迹）。点击跳转对端会话（实现可后置）。**只在跨会话时出现**：同会话 spawn 两端都在图内、虚线 spawn 曲线已表达关系（场景 10），不加 ↗——角标替代的是"画不出的那条边"，不是 spawn 的通用装饰 |
...
| 10 | spawn 派活 → attach 回流 | spawn 边虚线曲线 `5 4`（子分支色）从发起轮下缘出发；子 agent 头与 spawn 调用节点**同一行**、自己的 lane、tier=1——除非该行已被另一个头认领（第一节）；回流长虚线 `4 4` 从子分支 tip 拉回主分支嵌入位置（聊天流里渲染成 Spawned 卡片，显示序提前——见 `ui/invariants.md` 规则 9） |
...
虚线 spawn 曲线从子代理的回复节点出发，挂在子代理的 lane 结构下）照样画得出来。
...
一摞"——胶囊展开时它们消失，因为那时这摞被画出来了。旁边一行注文灰的注记——
`已压缩 · 14 轮`——让这个折叠自己把自己说清楚。药丸是字形，注记是这个字形的
含义；它画在形状之外，理由和子 agent 的名字一样（规则③）。
...
**双环点**。spawn 分支根画成一对同心圆，用分支色：外圈半径 `AGENT_DOT_R`，内圈
`AGENT_DOT_INNER_R`。它刻意不取用角色形状表里的任何一个——挂在它下面的是另一个
agent 的整段对话，而一个读起来像本链某一轮的字形说的恰恰相反。第二圈说的是"另
一条链"，不是"另一个节点"。

**名字是注文，不是形状**。它落在点右侧 `AGENT_CAPTION_DX` 处，注文灰、10px，随
节点一起在 hover 时提亮。折叠时带上身后那条链的规模——`后端架构 (14)`；展开时不
带，因为那些轮次已经在屏幕上、数得出来。超过 190px 的名字截断加省略号。

这是规则③在起作用。早先那颗药丸是按自身文字定尺寸的形状，每条可能落在它上面
的线都需要一套不对称裁剪好让线不横穿名字；点的边线像任何字形一样按自身半径裁。
注文的墨迹仍然存在，布局也仍然为它记账——但记的是一个数，不是一个形状：
`pipeline.ts` 量出绘制层将要画的那行注文、把它的右向延伸戳在头节点上
（`_spawnHW`），包围盒延到它那里让 fit 永不裁掉名字，而注文和 fork 桥这两种横贯
整行的墨迹各自认领独占的行（第一节）。列从不为文字预留：名字从行认领保证为空
的格子上方飞过，lane 间距和图上其他一切一样保持一到两个单位。

**spawn 边**。子分支色的虚线曲线（`5 4`），从发起轮的**下缘**出发，落在点的左缘。
从下缘而不是右缘出发，是因为 llm 节点的右边是它的 ⚒N / ×N 执行计数，从那里画的线
会横穿计数；落在左缘，是因为点的右边是名字。

**折叠**。分支默认省略；点这个点画出整条 lane，再点收起。嵌套的子 agent 保留自己
的点，不会消失进父 agent 的折叠里——否则就没有把内层打开的把手了。这个状态只属于
视图、从不持久化，和第九节完全一致。

**名字在线上从哪来**。来自 runner 戳在回指该分支的 attach 指针上的 label（见
dag/overview.zh.md 第四节），没有时回落到记录下来的分支名，都没有时写"sub-agent"。
检查器把同一个节点标题写作 `子 agent · <名字>`，而不是它 role 字段所说的 `user`——
那是字段的说法，不是这个节点的身份。

**HEAD 永不被折走**。checkout 到某个子 agent 的 lane 时，即使其余的点都关着，这条
lane 也保持画出。藏起用户当前所站位置的图，比画得太多的图更糟。

这个 pass 跑在压缩折叠之后（`passes/fold-spawn-branches.ts`），两者因此可以叠加而
互不知情：各管一种省略，被两者同时丢弃的节点只丢一次。
...
| 第一节 lane / tier / depth 布局 | `dag/layout/geometry.ts::computeGeometry`（按 tier 打包 lane、前序分行、头节点 + fork 根行认领）；格点性、无重叠、行认领、紧贴打包、包围盒盖注文均由 `web/scripts/check-dag-subagent.mjs` 真实执行并断言 |
| 第二节 规则③ 字形占格 | 没有任何形状按文字定尺寸——注文的实测延伸是布局的一个数（`_spawnHW`），不是字形宽度 |
| 第四节 HEAD 实心、无光环 | `render/nodes.ts` 把 `isHead` 作为 `_buildShapeEl` 的 `solid` 传入；覆盖点从实心里挖出；`right-dock.css` 已无 `.is-head` 光晕 |
...
| 图内分支标签（checkout 按钮） | `render/badges.ts`；hover 样式在 `right-dock.css` 的 `.history-branch-tag` |
...
| 第十二节 子 agent 点 | `shapes.ts` `agent_dot`（双环）与 `agentCaption`、`passes/fold-spawn-branches.ts`（折叠 + 取名 + HEAD 豁免）、`render/nodes.ts`（注文、`data-spawn*`）、`render/edges.ts`（从调用者下缘出发的虚线 spawn 曲线）、`store/globals.ts` 的 `_spawnExpanded`；由 `web/scripts/check-dag-subagent.mjs` 实跑 |
