...
**胶囊的位置**。它存储的 `predecessor` 就是覆盖区间自己的起点——即 ROOT，
因为压缩吃的永远是会话开头的前缀。折叠时（在携带它的分支上），胶囊站在覆盖
区间原来的位置：主干首位、继承幸存者的 lane，因为摘要是下一次请求携带的第一
样东西。展开时区间回到屏上，胶囊读作这些轮次收拢*成*的结果：它接在本分支的
对应位置——最后一个幽灵之后、保留尾巴之前：幽灵 → 胶囊 → 尾巴。两种摆位都是
`fold-summaries.ts` 里仅视图层的克隆改写；存储行在两种状态下都保留真实的
`predecessor`。
