...
delta 调校。鼠标滚轮按离散格档识别（line 模式的 delta，或 `deltaX = 0` 且
`deltaY` 为整格大小的整数），按滚轮速率缩放：鼠标没有捏合。其余就是触控板
双指滚动，做平移、两轴都动——滚动就是滚动。
...
（第十二节）同一套词汇，展开即消失，因为那时这摞被画出来了。药丸旁一个词的
注文灰注记——折叠时 `已压缩`、展开时 `展开中`——点明状态；它画在形状之外，
理由和子 agent 的名字一样（规则③）。
