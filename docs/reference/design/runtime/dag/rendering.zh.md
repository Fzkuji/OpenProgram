...
| 调用线程（锚点 → 它的条目，第十二节） | 点线 `2 3` 折线——先从锚点横跨一步，再沿线程列竖直向下（和全图一样正交） | 注文灰 | 展开时显示 |
...
**展开就是纯展开**。点节点把线程插进布局：一条淡点折线（`2 3`）从锚点横跨到
它右侧第一个空列再竖直向下，每个事件都是线上的真节点——每次调用一个方块（锚轮 lane 色），spawn
