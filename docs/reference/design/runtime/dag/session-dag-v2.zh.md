...
状态：裁决 1 已实现（e1293cc0，2026-08-02；切换时旧会话数据已整体删除）。
裁决 2-4 已批准，待实现。

裁决 1 实现期敲定的语义：
- 会话首节点和显式 root fork 统一写哨兵值 `predecessor="ROOT"`（不是留空）——
  重试第一条消息会产生合法的 ROOT 级兄弟节点，写入不变量必须放行它。
- 除设计中的两个例外，ask_user 应答节点和压缩节点（`summary_`/`k_`）在
  裁决 4 收编 compaction 之前也豁免于不变量。
- spawn 分支的 `get_branch` 现在停在 spawn 根（此前会经 caller 边漏进父分支）。
  与干净上下文的 spawn 语义一致；spawn 分支的聊天视图不再显示父分支历史。

—— 原设计稿正文如下。与 `session-dag.md`
