...
The sandbox is the **host-native process isolation layer**: Seatbelt on macOS and bubblewrap on Linux constrain the filesystem, process view, environment, and network of a child process. The fixed `owner`/`paired` authority-tier table, permission rules, and exact owner approval decide whether an operation may be attempted; `SandboxPolicy` decides what an admitted process can access; hard constraints run before either layer ([`permission-model.md`](permission-model.md) §1.1). An approved retry and `permission_mode="bypass"` both run under `escalated_policy`, which relaxes the configurable restrictions but remains inside the OS sandbox and cannot remove credential filtering or the hard floor.
...
| `sandbox.apply_in_bypass` | keep the configurable sandbox limits enforced under `permission_mode="bypass"` | `false` |
...
**Only a local interactive owner can request one exact retry with relaxed configurable restrictions.** The rerun still uses the OS sandbox, preserves credential filtering and the non-configurable agentics write prohibition, and never applies to cron, subagents or shared channels. `permission_mode="bypass"` takes the same path: matching Claude Code's `--dangerously-skip-permissions`, bypass means full access — every tool call in the bypass branch runs under `escalated_policy`, so the configurable restrictions (`deny_read`, `deny_write`, network) do not apply, while the hard floor and credential filtering remain. Setting `sandbox.apply_in_bypass=true` keeps the configured sandbox enforced even under bypass.
...
**3. Switch semantics — done.** The `ContextVar` is gone. The policy is resolved from `sandbox.*` in the config at the moment a command is wrapped, which survives the asyncio-task-to-thread hop, the `spawn` subprocess and a nested CLI, because a file does not belong to a context. It also sits below the permission layer: `permission_mode="bypass"` short-circuits the approval card, and by default runs under `escalated_policy`, so the configurable sandbox limits do not apply in bypass (the hard floor does); `sandbox.apply_in_bypass=true` restores the configured sandbox under bypass. `wrap_command` takes an explicit policy for callers that hold one; nothing yet supplies a different policy per tool or per call site, which is what "the call site can override" was meant to buy. An unavailable backend refuses by default and names the two ways out.
...
**Visible state.** The chat top bar's permission badge gains a sandbox indicator, and the `bypass` label says what the mode means: "Bypass permissions", described as full access with no approval prompts and no sandbox limits — matching the runtime semantics, where bypass runs under `escalated_policy` unless `sandbox.apply_in_bypass=true`. The incident's owner confusion ("I enabled bypass, why is there a sandbox?") is resolved at the semantics level, not only the caption level.
...
- Hard constraints and the fixed authority-tier capability check run before permission rules, approval and `permission_mode="bypass"`. The bypass branch itself runs under `escalated_policy` — full access to the configurable surface, hard floor retained — unless `sandbox.apply_in_bypass=true` keeps the configured sandbox.
