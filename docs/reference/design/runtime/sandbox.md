...
The sandbox is the **host-native process isolation layer**: Seatbelt on macOS and bubblewrap on Linux constrain the filesystem, process view, environment, and network of a child process. The fixed `owner`/`paired` authority-tier table, permission rules, and exact owner approval decide whether an operation may be attempted; `SandboxPolicy` decides what an admitted process can access; hard constraints run before either layer ([`permission-model.md`](permission-model.md) §1.1). An approved retry runs under `escalated_policy`, which relaxes the configurable restrictions but remains inside the OS sandbox and cannot remove credential filtering or the hard floor. Default `permission_mode="bypass"` uses `unsandboxed_execution` instead: `_execution_policy_override` is `None`, so commands run unwrapped, the environment is not stripped, and the hard floor is not applied — matching Claude Code's `--dangerously-skip-permissions`. `sandbox.apply_in_bypass=true` keeps the configured sandbox under bypass.
...
The policy implementation is `openprogram/sandbox/__init__.py`. Its public names include `SandboxPolicy` (a frozen dataclass), `resolve_policy()`, `is_available()` / `unavailable_reason()`, `child_env(policy)`, `validate_write_path()`, `wrap_command(command, cwd, policy) -> (args, shell)`, `escalated_policy()`, and `unsandboxed_execution()`. `openprogram/backend/local.py::_invocation` is the shared command boundary used by bash, process, local `execute_code`, cron direct jobs, the memory writer's MCP `shell`, and one-shot MCP startup. Spawned agentic processes receive an explicit policy snapshot. The direct `write`, `edit`, and `apply_patch` tools apply the same writable-root and hard-floor checks without converting file operations into shell commands.
...
| fork | allowed |
| Signals, process info | Signals stay `(allow signal (target same-sandbox))` in every policy. Process info is `(allow process-info* (target same-sandbox))` by default; `escalated_policy` sets `host_process_info=True` and emits `(allow process-info*)` so the retry can inspect host processes |
...
| PID namespace | unshared by default (`--unshare-pid`); omitted when `host_process_info=True` |
...
On Linux the environment filter alone would not be enough. The default policy therefore passes `--unshare-pid`: without it, `/proc/<agent_pid>/environ` hands back the key that was just removed from the child; with it, the sandboxed process sees four PIDs, `cat /proc/<host pid>/environ` reports "No such file or directory", and `kill -9 <host pid>` reports "No such process" while the host process stays alive. `escalated_policy` sets `host_process_info=True` and omits `--unshare-pid` so the approved retry can inspect host processes; credential names still stay out of the child environment.
...
| `sandbox.apply_in_bypass` | keep the configured sandbox under `permission_mode="bypass"` (default bypass is unsandboxed) | `false` |
...
**Only a local interactive owner can request one exact retry with relaxed configurable restrictions.** The rerun uses `escalated_policy`: the OS sandbox stays on, credential filtering and the non-configurable agentics write prohibition remain, and the retry never applies to cron, subagents or shared channels. `escalated_policy` also sets `host_process_info=True` so the retry can inspect host processes. If the denied execution already ran under an escalated or unsandboxed policy (`already_escalated`), the denial is returned as-is and no upgrade-retry card is shown; the `sandbox.violation` observation event still fires.

**`permission_mode="bypass"` is a different path.** Matching Claude Code's `--dangerously-skip-permissions`, default bypass means no sandbox: `_gated_execute` wraps the call in `unsandboxed_execution`, so `resolve_policy()` returns `None`, `_invocation` does not wrap the command or strip the environment, and the hard floor is not applied. The call is marked `already_escalated=True`, so a later sandbox denial (for example from a `force_sandbox` caller) does not prompt for an upgrade retry. Setting `sandbox.apply_in_bypass=true` keeps the configured sandbox enforced even under bypass; that path is not already-escalated, so a structured denial can still request an `escalated_policy` retry.
...
`permission_mode="bypass"` no longer precedes security enforcement. Hard constraints, rule-layer deny/ask, and capability checks execute first. The bypass branch itself runs under `unsandboxed_execution` (no wrap, no environment strip, no hard floor) unless `sandbox.apply_in_bypass=true` keeps the configured sandbox. Subagents are non-interactive, inherit no more than the caller's scope, and cannot request escalation. The ignored provider keyword `sandbox="read-only"` is not counted as protection; process isolation is supplied by the explicit policy snapshot and the boundaries above.
...
**3. Switch semantics — done.** The `ContextVar` is gone. The policy is resolved from `sandbox.*` in the config at the moment a command is wrapped, which survives the asyncio-task-to-thread hop, the `spawn` subprocess and a nested CLI, because a file does not belong to a context. It also sits below the permission layer: `permission_mode="bypass"` short-circuits the approval card, and by default runs under `unsandboxed_execution` (unwrapped, full environment, no hard floor); `sandbox.apply_in_bypass=true` restores the configured sandbox under bypass. `wrap_command` takes an explicit policy for callers that hold one; nothing yet supplies a different policy per tool or per call site, which is what "the call site can override" was meant to buy. An unavailable backend refuses by default and names the two ways out.
...
*Backward:* a structured sandbox denial can request one exact local-owner approval, unless the denied execution already ran under an escalated or unsandboxed policy (`already_escalated`). The retry uses `escalated_policy` rather than unrestricted host execution; the agentics hard floor and credential filtering remain active, and `host_process_info=True` lets the retry inspect host processes.
...
The profile patch list is complete: the working directory is escaped before interpolation, signals stay same-sandbox in every policy, process inspection is same-sandbox by default and host-wide under `escalated_policy` (`host_process_info=True`), Linux receives the namespace and capability flags above (and omits `--unshare-pid` when `host_process_info` is set), both paths use `/bin/bash`, macOS sysctl access is narrowed, general `mach-lookup` is removed, and temporary writes are limited to the current `TMPDIR`. `--unshare-user` remains absent because the non-setuid bubblewrap build creates a user namespace itself and the setuid build rejects the flag.
...
**Visible state.** The chat top bar's permission badge gains a sandbox indicator, and the `bypass` label says what the mode means: "Bypass permissions", described as full access with no approval prompts and no sandbox — matching the runtime semantics, where default bypass uses `unsandboxed_execution` unless `sandbox.apply_in_bypass=true`. The incident's owner confusion ("I enabled bypass, why is there a sandbox?") is resolved at the semantics level, not only the caption level.
...
What this deliberately does not change: the two-layer architecture (approval decides, sandbox confines), the deny-before-bypass ordering, the hard floor on approved `escalated_policy` retries, and the loaded defaults. Default bypass is unsandboxed and sits outside that hard floor. The design adds no second enforcement point; every part above is presentation, persistence, or prompt.
...
- Hard constraints and the fixed authority-tier capability check run before permission rules, approval and `permission_mode="bypass"`. The bypass branch itself runs under `unsandboxed_execution` — unwrapped, full environment, no hard floor — unless `sandbox.apply_in_bypass=true` keeps the configured sandbox.
...
- Sandbox denials are structured. Only a local interactive owner can approve one exact retry under `escalated_policy`, which retains the hard floor and credential filtering and sets `host_process_info=True`. A denial that already ran under an escalated or unsandboxed policy (`already_escalated`) is returned without an upgrade-retry card; the `sandbox.violation` event still fires. Persistent approval stores the normalized exact operation; complex shell expressions are once-only.
- Nested Claude Code built-ins are disabled and replaced by managed MCP file and shell tools.

Section 11 (denial legibility and negotiation) is implemented as of 2026-08-22 except two pieces: denials name the matched rule and blocked path, `sandbox.allow_read` exists and is enforced at all three points (Seatbelt, bubblewrap, in-process read tools) with narrower-path-wins and a hard floor that no allow entry can open, the escalation card offers "Always allow this path" (reply scope `always_path`, persisted into `sandbox.allow_read`), the bypass label reads "Bypass permissions" with a full-access description, and the system prompt carries the no-relocation line. Bypass semantics align with Claude Code's `--dangerously-skip-permissions`: the bypass branch executes under `unsandboxed_execution` by default, and `sandbox.apply_in_bypass` (default false) opts back into the configured sandbox under bypass. Still open: the badge suffix is static rather than driven by live sandbox state (the web UI has no sandbox-state feed yet), and the Security presets panel does not exist.
