...
**LLM judge** — the `goal_judge` agentic function (`openprogram/functions/agentics/goal/`): the framework eats its own dog food, so the judgment point is an `@agentic_function` whose docstring IS the judge prompt, runnable standalone from the Functions panel. It makes one no-tools call (`runtime.exec(toolset="none", response_format=<JSON schema>)`) and returns the parsed `{"met", "reason", "need_user", "question"}` dict, raising on a malformed reply. `goal.py` keeps the deterministic side: it builds the runtime on the session's configured model (profile + per-session override, resolved through `internals/_model_tools` and stamped onto a `Runtime` in `_judge_runtime`; the provider registry's `fast` flag is a speed tier of the same model, not a cheaper judge model, so there is no separate judge model to pick) and retries a failed attempt once within the same evaluation. Input is the goal text plus a tail render of the active branch: the last 8 messages' content plus each assistant row's persisted tool blocks, clipped per-field and capped at ~24 k chars. The tail is rendered in `goal.py` rather than by `render_session_transcript` because the stock transcript keeps the head and drops later turns — the wrong end for judging recent progress.
...
**Stop verdicts are actively verified.** The tail judge reads only the working model's own narrative — it can neither check a claim nor escape the transcript's framing. Feeding it more transcript fixes neither. The errors are also asymmetric: a wrong "keep going" costs one extra turn, a wrong STOP (false completion, needless interruption) is the expensive one. So `met` and `needs_user` verdicts from the tail judge get a second opinion before the loop acts on them: the `goal_verify` agentic function (same module as `goal_judge`, prompt in its docstring, panel-runnable) spawns a same-session agent turn (inspection-only tools — `bash`, `read`, `grep`, `glob`, `list`; `advance_head=False`; anchored on the verified turn so it renders as a sub-agent square) that receives the goal and the CLAIM but **no transcript**, and gathers its own evidence from the working directory — run the tests, read the files, look at the artifacts. Confirmed → the stop proceeds with the verifier's evidence as the reason. Refuted → the verdict becomes unmet with the verifier's gap ("核实未通过：…") feeding the next continuation. `goal_verify` fails OPEN — a verifier failure or an unparseable reply returns `confirmed=true`, falling back to trusting the tail judge (a verification hiccup must not brick the loop). Ordinary continue verdicts pay nothing.
