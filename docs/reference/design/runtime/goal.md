...
**Unattended and undecidable — the full resolution chain.** When nobody is watching, "can't decide" resolves in one of two ways, and only these two:

1. *Decidable but uncertain* (ambiguity, repeated failures, style choices): the agent must decide itself — pick the most reasonable plan, record the decision and its reasoning in the transcript, and continue. This is enforced twice: the judge prompt forbids pausing for these in unattended mode, and the tool layer independently strips `ask_user_question` from every unattended turn (`denied_ask_tools` in `agent/attended.py`, folded into the tool-policy deny set during resolution), so the working agent cannot ask even if a prompt slips.
2. *Truly blocked* (missing credential/resource/access, or an irreversible/destructive action that must not run unapproved): the run parks. Status goes `waiting_user`, no continuation launches (no spinning, no budget burn), the question lands as a system row in the transcript ("[goal] 需要你的确认才能继续：…") and on the goal chip, and the next real user message is treated as the answer — the loop resumes and judges as usual.

The invariant: an unattended run never guesses its way through an irreversible action, and never loops idle on an unanswerable question — it either decides-and-records or parks-and-waits. Known ceiling: a parked question is only visible in the session itself; there is no push notification channel yet, so the user discovers the pause on their next visit.
