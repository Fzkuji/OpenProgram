...
- **Web**: `session_loaded` carries the goal (`ws_actions/session.py`) for hydration; the composer's `GoalChip` (`web/components/chat/goal-chip.tsx`, shared `useSessionGoal` hook) renders `◎ goal · N/M` from that plus live `goal_update` frames (delivered through `use-ws`'s catch-all `op:ws-message` event). While `status === "waiting_user"` the composer itself expands into **question mode** — the same surface `ask_user_question` uses: the parked goal synthesizes a `PendingDecision` (real pending asks take priority), the header badge reads "goal · 等你回答" with the Target icon, options render as pills with their descriptions, and free text stays available. Answer submission bypasses `question_reply`: the picked label (or typed text) goes through the normal chat send path — exactly a typed message, so the loop's resume rule needs nothing special. The mode is driven by goal state, so a reload while parked re-enters it. Composer-typed `/goal …` is executed backend-side by the local-builtin branch in `ws_actions/chat.py`: a status/clear reply returns as a `local_command` envelope rendered as a transient system row; a set replaces the turn text with the goal directive and falls through into the normal turn flow.
