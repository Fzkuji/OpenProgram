...
- Each continuation is a first-class turn: `continue_goal_turns` calls `_process_turn_once` (never `process_user_turn`, so the loop cannot nest) with a `TurnRequest` built by `dataclasses.replace` — `source="goal_continue"`, `user_text="[goal] 未达成：<reason>。继续。"`, fresh `user_msg_id`, `branch_from=INHERIT_PARENT`; model / permission / tool settings carry over from the triggering request. The turn gets its own persistence, git commit, and compaction, mirroring the task runner's follow-up construction (`agent/task/runner.py`).
