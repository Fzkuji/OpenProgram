...
Judge-side enforcement: on an anchored goal the judge MUST open the deliverable (and the reference when accessible) and walk the reference-derived criteria item by item; the working agent's "I have completed…" narrative is never sufficient evidence for `met=true`. This closes the main early-exit failure mode — quality requirements stated only in the first task message get compacted out of the session view, while the anchor lives in the spec the judge re-reads every turn.
