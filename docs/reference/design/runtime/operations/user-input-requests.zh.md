...
| 机制 | 状态 |
|---|---|
| `ask_user` / `set_ask_user` / `FollowUp`（`openprogram/programs/agentic_functions/ask_user/`） | 原语完整，DAG awaiting-node 簿记完整；但 worker 里没注册 handler，且 agentic 子进程桥是单向的——实际上返回 `None` |
