...
| Mechanism | State |
|---|---|
| `ask_user` / `set_ask_user` / `FollowUp` (`openprogram/programs/functions/agentic/ask_user/`) | Primitive complete, DAG awaiting-node bookkeeping complete; but no handler registered in the worker, and the agentic subprocess bridge is one-way — returns `None` in practice |
