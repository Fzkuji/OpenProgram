...
`list_sessions` 和 `list_branches`（`functions/tools/send_message/`）已经能列出会话 id 和 `SID:HEAD` 分支端点。这正是 `session_transcript` 要的参数，所以不新增列表工具。工具接受整串 `SID:HEAD` 传进 `session_id` 并自行拆分，因为那就是 `list_branches` 打印的形式。
