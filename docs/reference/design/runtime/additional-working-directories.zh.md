...
跟随既有文件风格：
- `tests/unit/store/sessions/test_session_config.py`：`additional_working_dirs` save/load 往返（含 None 不动、`_as_str_list` 清洗）。
...
- `tests/unit/store/sessions/test_session_main_workdir.py`：主目录那侧的相反规则——定格、目录缺失时的解析、relocate 记录节点——外加把两者串起来的那条：主目录已定格的会话上照样增删额外目录。
