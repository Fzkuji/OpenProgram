...
判定侧强制（不限于锚定）：只要 spec 里有可核验项——checklist、可数阈值、必须存在的文件、必须通过的命令——裁判**必须**先用工具逐项取证再答 `met=true`；带锚的 goal 还要打开参考确认逐项达到或超过。干活 agent 的"我已完成……"叙事只能用来判那些工具核不了的项。这堵住了提前收工的主要漏洞——只写在首条任务消息里的质量要求几轮压缩后就从会话视图里消失了，而锚写在裁判每轮都重读的 spec 里。
