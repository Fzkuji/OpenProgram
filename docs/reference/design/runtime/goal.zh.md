...
1. *能决定但拿不准*（歧义、反复失败、风格选择）：agent 必须自己决定——选最合理的方案，把决定和理由写进会话记录，继续。这是双重强制：判定 prompt 禁止无人值守下为这些暂停；工具层独立把 `ask_user_question` 从所有无人值守轮次里扣掉（`agent/attended.py` 的 `denied_ask_tools`，在工具解析时并入策略 deny 集），就算 prompt 失守也问不出去。
2. *真正无法继续*（缺凭据/资源/权限，或必须有人批准的不可逆/破坏性操作）：运行挂起。状态置 `waiting_user`，不再发续轮（不空转、不烧预算），问题作为系统消息落进会话（"[goal] 需要你的确认才能继续：…"）并显示在 goal 徽标上，用户回来后的下一条真实消息就是回答——循环恢复、照常判定。
