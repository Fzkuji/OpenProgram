...
3. **Hand-written HTML**: complete pages retain their layout, styles, scripts and visualizations in an iframe backed by a `.raw.html` sibling. Body-only fragments receive scoped styles inside the site shell. Both paths resolve internal document links through the shared link rewriter, including relative `.md` links in English and Chinese pages. Source documents keep their relative `.md` links; built links target the corresponding site `.html` pages.
