...
| Apps migration B2: Python CLI application | `2a790501`, `91be4353`, `51303ea8`, `09eb3732`, `252fb4fa`, `a459c443` | implemented; reviewed | Moves parser, dispatch, Rich fallback, Ink launcher and setup flows into the installable `openprogram_cli` application package. `openprogram.cli` remains a bounded compatibility loader; editable and wheel installs use the application package. The later root cleanup removes the temporary raw-checkout canonical-module forwarder. |
...
| Standard polyglot workspace G2 | G2a release-tool grouping; G2b root npm workspace; Core-layout rollback `24f72a39`, `5c04bb1b`, `aa0e82ea`, `defacac1`, `ddfe06a6` | implemented; reviewed | Keeps applications under `apps/`, the Agent Core at the repository root `openprogram/`, one aggregate Python distribution, one npm workspace lock, and formal distribution commands under `scripts/release/`. The proposed `packages/core/src/openprogram` migration was rejected and fully reversed because the repository has only one Core Python package and the added directory depth did not establish a useful package boundary. |
...
- The G2c Core move is not part of the accepted structure. The Core remains at
  `openprogram/`, so raw source-checkout imports, installed imports, module and
  console entries, package data and the aggregate wheel retain their original
  layout. `packages/` is not created until the repository contains a second
  genuine shared package with an independent ownership boundary.
...
### Core-layout rollback evidence

- Reverts `24f72a39`, `5c04bb1b` and `aa0e82ea` restore the complete tracked
  Core to `openprogram/`, restore the original package discovery and checkout
  paths, and remove `packages/`.
- The local GUI, research and wiki Harness checkouts, Agentic Programming logs
  and mutable Programs metadata were returned to their original locations
  under `openprogram/`. Only generated bytecode and Finder metadata were
  removed from the obsolete directory tree.
- Focused rollback verification passed `69` Python tests and `10` CLI tests
  with `2` environment skips. A clean Git checkout passed `57` tests with the
  optional local Harness test explicitly skipped. Root imports, both retained
  CLI module entries, wheel, sdist and documentation links pass; package
  artifacts contain `openprogram/` and no `packages/` prefix.
- Specification review: PASS. Quality/Ponytail full review: PASS. No src-layout
...
- Main integration preserves the already verified provider, Programs explorer
  and Agent-management changes in `566f0a3d`, `47b1b407` and `f736cb14` in the
  root Core. Obsolete unreferenced promotion drafts were removed in `5c041a1b`.
- Integration repair `e310ecf0` removes the retired `promo/` directory from
  the authoritative layout and makes `node_modules/` ignored at every
  workspace depth. The later Core rollback restores the CLI owner-auth test to
  the root `openprogram/` path.
...
- Rollback specification review: PASS. Rollback quality/Ponytail full review:
  PASS. Remaining integration concerns: none.
