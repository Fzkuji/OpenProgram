...
| Backfill of pre-marked sources | Implemented and executed on the live workspace (8 Topics, 132 blocks, all 23 sources cited, 51 citations verified unbroken) | `openprogram memory backfill` runs the writer over every trusted source no Topic cites, ignores node markers, excludes pending sources, and promotes a legacy root `core.md` into `topics/core.md` rather than discarding it. It is idempotent against existing citations and resumable after a failed batch. |
