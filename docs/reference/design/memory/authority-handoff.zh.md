...
| 已标记来源的 backfill | 已实现并在实机 workspace 执行完毕（8 个 Topic、132 个 block、23 个 source 全部被引用，51 条引用逐条校验无断裂） | `openprogram memory backfill` 对所有未被 Topic 引用的 trusted source 跑一遍写入器，忽略节点标记，排除 pending 来源，并把遗留的根 core.md 提升进 topics/ 而不是丢弃；对已有引用幂等，批次失败后可续跑。 |
