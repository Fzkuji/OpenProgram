...
一轮带两样身份，因为任何一样单独用都会丢人。显示名在主题文件里读起来自然，
也是按人名检索时能命中的那个词，而人会改名，两个人也会重名。平台id经得住
改名，也能把两个都叫Ada的人分开，而满篇数字的文件对读它的人什么也没说。
所以`SourceRecord`同时带显示名和id，渲染出的标签是`显示名(id)`。

标签走的是现成的槽位。写入prompt把一批轮次渲染成`[ref] speaker: text`，
sources归档写的是同一个形状，所以变的是speaker槽位里的值：这一轮有身份就
放标签，没有就放原本的role。网页、命令行、TUI的轮次背后没有渠道身份，保持
role不变，那正是写入agent今天看到的样子。写入prompt里写明：一行前面的名字
是说这句话的人，于是事实落到那个人名下，而不是落到"用户"名下。

发信人的平台id从渠道适配器送到轮次上，跟已经在走这条路的显示名同行。它跟
`peer_id`是两个字段：`peer_id`是路由目标，也是回复要发回去的地址，群聊里
它等于群。显示名进来时压成一行，因为一条记录一行是两个渲染出口共同的前提，
而显示名是它的主人在平台上随手填的任何东西。

身份是记忆记录的内容，不用来切分记忆。一份工作区、一套主题文件，账号批准
的所有人共用，团队机器人的价值正在于此。想要自己那份记忆的人跑自己的实例
...
- **只有head所在的那条分支会被拿出来。**两条写入路径要的都是终止于会话
  head的分支，所以用户在过阈值之前就离开的分支，再也不会被回访：没有任何
  一次行走会再到它的末端。游标让写多条分支变得安全，但它不会去找分支。去
  遍历`list_branches`可以，代价是把用户有意放弃的分支也写进去。
...
除了"谁说的"和"写入游标"两节，上面的内容都已经在跑。

"谁说的"已定稿，还没落地。`scriptorium/writing.py`的`_records`只读行上的
`role`，别的什么也不读，于是共用一通会话的每个人都被记成`user`。身份已经
到了一半：`peer_display`由`dispatcher/prep.py`逐条写在user行上，`get_branch`
读回来还在，用真实的`SessionDB`验过。发信人自己的平台id停得更早，止步于
`channels/base.py`的门禁，再往后没有；轮次上的`peer_id`是路由目标，群聊里
等于群。方案落在七个文件，四个负责把`sender_id`从渠道适配器送下来，三个负责
把身份用起来，图解见[`speaker-identity.html`](speaker-identity.html)。
