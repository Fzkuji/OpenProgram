...
> 本文是记忆子系统的权威设计: 两个层级、连接两级的 provenance 指针、由实体层派生虚拟层的
> 提炼管道, 以及把它们送到模型面前的召回路径。实体层的 git 底座细节见
> [`git-as-entity-memory.md`](git-as-entity-memory.md); 本设计取代的线性总结链见
> [`memory.md`](memory.md)。
>
...
线性总结链 (`raw chat → 抽 0-10 facts → journal → wiki → core`) 每一层都丢信息,而且抽象层是从上一层的有损总结来的,不是从源头来的。这样实体层和抽象层就成了两套不相通的东西。本设计让实体层成为抽象层唯一读取的数据源,并让每条派生记录都保留一个指回源头的指针。
...
1. **Git 作为 episodic memory 的底座**。实体记忆不是自研存储,直接用 git: commit 不可变,记录无法被篡改; log = 时间线; checkout = 时光机; branch = 探索过的分支; 而且 agent 能用标准工具 (`git log` / `grep` / `diff`) 自己读。可审计、可复现、可回溯。

2. **Provenance-pointer 索引,而非替代**。虚拟层不取代实体层,而是给它建一个**带坐标的导航地图**。每条虚拟记忆都挂一个指针 `(project, session, commit, timestamp)`,指回它在实体层的出处。这解决了"有损总结丢上下文"的根本问题——**任何时候都能顺着指针钻回源头记录**。
...
### 3.1 每个会话都属于某个 project

**没有"无主"的会话**。每个会话都属于一个 project,区别只在于这个 project 是不是用户真实的工作目录。
...
会话存储在 `<state>/sessions/<id>/` (`store/git_session.py`)。每个节点是 `Call` (role = user / llm / code),边是 `called_by` (调用链) + `reads` (上下文引用)。`meta.json` 里的 `project_id` 字段记录会话的项目绑定。

绑定了真实项目的会话,其 repo 不在 home 根,而在 `<项目>/.openprogram/sessions/<id>/`,由 `sessions/locations.json` 索引指向 (`SessionStore._record_location` / `_session_dir`)。这样一个项目的代码历史 (project-git) 和对话历史 (session-git) 都待在项目目录里,跟着项目走。
...
**自动 commit**: turn 结束时,若 session 绑了真实 project 且 agent 改过文件:
...
只有当会话绑定了真实工作目录时,才会出现一个真 git 仓 (见 §3.4)。即: **真实路径 → 真 repo;没路径 → 只有标签**。这避免了实体层出现一堆空的 default 仓这种无意义膨胀。

## 4. 虚拟层 (Virtual Memory)

两个 projection,都从实体层提炼,都带 provenance 指针,都 bi-temporal。

### 4.1 Provenance 指针 (核心数据结构)
...
实体 + 关系。回答"什么和什么是什么关系"。相比只有孤立实体页的 wiki,图额外表达边和时间。
...
- **压缩前 flush**: context 压缩前插一轮,让 agent 把还在对话里的关键信息先落实体层
...
管道**直接读 session-git 里的 `Call` DAG**——包括 `code` 节点 (agent 跑了什么工具、什么参数、什么结果) 和 `reads` 边 (什么影响了决策)。这些正是图谱投影需要的材料 (`agent ──ran──► pytest ──produced──► 3 failures`),如果管道读的是已经压扁的对话文本,这些信息就丢了。

## 7. Schema 总览

```
~/.openprogram/
├── sessions/<id>/               实体: 会话 (meta 带 project_id)
...
**复用**:
- Session-Git (`<state>/sessions/`, `store/git_session.py`) — 实体层第一块
- Project-Git (`store/project_store.py`) — 实体层第二块
- `Call` DAG (`context/nodes.py`) — 实体层的节点模型
- `MemoryProvider` 抽象接口 (`memory/provider.py`) — 召回 hook 形状
- sleep 调度器骨架 (`memory/scheduler.py`, `sleep/runner.py`) — 读 git
...
- `memory/journal/` 日记层 → timeline (原始记录已在 session-git)
...
评估方向: context 占用 vs 召回准确率的 trade-off; 多会话长程一致性; 矛盾检测召回率; 跟 RAG / Zep / mem0 baseline 对比。

## 附录: 实现状态

实体层已就位: Project schema、会话绑定、project-git、会话落在项目内都已实现 (`store/project_store.py`、`store/session_store.py`)。提炼所需的读取层在 `store/session/provenance.py`。

§4 描述的虚拟层尚未建成。当前在跑的是 [`memory.md`](memory.md) 记录的线性 journal/wiki/core 链,它读的仍是 `get_branch()` 渲染出的对话文本,而不是 session-git 的 `Call` DAG;project-git 的 commit 历史完全没有被读取。接通这一跳是 §6 提炼管道的第一步。

召回 (§5) 目前注入的仍是 v1 的 core;§5.2 的导航工具尚未注册。UI 有顶栏项目选择器;Projects 面板、回溯 timeline 和 `/memory` 命令尚未建成。
