...
- **One key list per provider** (web `provider-keys.tsx` + REST `/accounts/{name}/{keys,keys/reorder,strategy,retry}`) — api-key providers show their key(s) as a single reorderable list instead of a single field + a separate rotation pool. Paste a key (validated on add), the top key is the default, reorder with ↑ / ↓, per-key health badge (valid / cooling / error), pick a strategy when there's more than one. A key set the old way (env var / config) is migrated into the list automatically.
