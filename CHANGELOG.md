...
- **Authoritative install docs** — [`docs/install.md`](docs/install.md) reworked into the host-model guide (full dependency matrix, troubleshooting); README / GETTING_STARTED (EN + 中文) now lead with the installer instead of `pip` (which only covers the Python core, not the web UI build or the GUI weight/OCR). `.gitattributes` pins `*.sh` to LF so the installer never breaks on macOS/Linux from CRLF.
