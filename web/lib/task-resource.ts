import type { TaskResourceView } from "./net/ws-events";

export type TaskResourceDetail = {
...
export function queueResourceSummary(
  resource: TaskResourceView | undefined,
): string | null {
  ...
  return [
    ...
    `${capacityValue(capacity.session_tasks)} tasks`,
...
export function taskResourceDetails(
  resource: TaskResourceView | undefined,
): TaskResourceDetail[] {
  if (!resource) return [];
  if (resource.resource_state === "legacy/unmetered") {
    const legacy: TaskResourceDetail[] = [
