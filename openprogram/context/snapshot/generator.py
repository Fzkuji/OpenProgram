"""Snapshot 增量生成 — 从上一个 snapshot 复制 + 加新节点 + 跑规则.

核心循环: 每个 turn 跑一次 generate_snapshot(), 产生新 snapshot.

不重算: items 是从 parent snapshot 直接复制过来的, 已 locked 的 item
...
def _gen_snap_id() -> str:
    return "snap_" + uuid.uuid4().hex[:10]
...
def _build_item_from_node(node: dict, snap_id: str) -> Optional[ContextItem]:
    ...
    return ContextItem(
        ...
        state_set_at=snap_id,
        reason="new",
    )


def _copy_item(item: ContextItem) -> ContextItem:
    """浅拷贝 ContextItem. dataclass 默认是 mutable, 不拷贝会被规则改到 parent snap 的 item."""
    return ContextItem(
...
def _describe_changes(items, parent_snap, snap_id: str) -> str:
    """生成 1 行变化描述, UI timeline 显示用."""
    from collections import Counter
    counts = Counter(i.state for i in items)
    new_count = sum(1 for i in items if i.state_set_at == snap_id and i.reason == "new")
