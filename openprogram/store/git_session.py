...
class GitSession:
    ...
    #
    # Multi-agent concurrent edits need each agent to materialize its
    # own branch in a separate directory, otherwise two writers race
    # on the same files in ``workdir/``. ``git worktree add`` gives
    # us exactly that: a sibling directory bound to a branch, sharing
    # the same object database. Sub-agent's ``commit_all`` commits to
    # its branch; the main session's ``workdir/`` is untouched until
    # an explicit merge turn (task F) consolidates branches.

    def worktree_root(self) -> Path:
        """Where per-agent worktrees live. Sibling to ``workdir/`` so
        ``git add -A`` on the main repo never sweeps them up (git's
        own worktree mechanism keeps them invisible to the main
        index)."""
        return self.path / "_worktrees"

    def add_worktree(
        self, branch_name: str, *, base_ref: str = "HEAD",
    ) -> Path:
        """Materialize ``<repo>/_worktrees/<branch_name>/`` bound to a
        new branch ``branch_name`` forked off ``base_ref``. Returns the
        worktree path.

        Idempotent at the path level — if the worktree dir already
        exists and is registered, returns it without re-running. The
        branch itself must not pre-exist (git refuses to bind two
        worktrees to the same branch); caller picks a fresh name.
        """
        self._ensure_init()
        root = self.worktree_root()
        root.mkdir(parents=True, exist_ok=True)
        wt_path = root / branch_name
        with self._lock:
            existing = self._run(
                "worktree", "list", "--porcelain", check=False,
            )
            if f"worktree {wt_path}" in existing:
                return wt_path
            self._run(
                "worktree", "add", "-b", branch_name,
                str(wt_path), base_ref, "--quiet",
            )
        return wt_path

    def remove_worktree(self, path: str | Path) -> None:
        """Tear down a worktree by path. The branch ref stays — caller
        decides whether to ``git branch -D`` it after merging or
        discarding. ``--force`` so a dirty worktree doesn't block
        cleanup; the caller is signalling intent to drop it."""
        if not self.exists():
            return
        with self._lock:
            self._run(
    ...
    def list_worktrees(self) -> list[dict[str, str]]:
        """Parsed ``git worktree list --porcelain`` output. Each entry
        carries at least ``path`` and ``branch`` (or ``HEAD`` if
        detached). Used by UI / cleanup logic to see which agent
