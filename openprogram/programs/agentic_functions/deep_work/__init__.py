...
from openprogram.programs.agentic_functions._utils import parse_json
...
def _clarify(task: str, level: str) -> dict:
...
@agentic_function(render_range={"callers": 0})
def _evaluate(task: str, standard: str, work_summary: str) -> dict:
...
def deep_work(
    ...
    if not state.get("clarified") and interactive:
        ...
        try:
            clarify_result = _clarify(task=task, level=level)
        finally:
