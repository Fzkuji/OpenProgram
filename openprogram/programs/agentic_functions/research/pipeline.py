...
Usage:
    from openprogram.programs.agentic_functions.research.pipeline import research_pipeline
...
def _stage_literature(project_dir, topic, runtime):
    from openprogram.programs.agentic_functions.research.stages.literature import run_literature
    if not topic:
...
def _stage_idea(project_dir, topic, runtime):
    from openprogram.programs.agentic_functions.research.stages.idea import run_idea
    if not topic:
        return {"status": "skipped", "reason": "no topic provided"}
...
def _stage_experiment(project_dir, runtime):
    from openprogram.programs.agentic_functions.research.stages.experiment import run_experiments
    return run_experiments(project_dir=project_dir, runtime=runtime)


def _stage_analysis(project_dir, runtime):
    from openprogram.programs.agentic_functions.research.stages.writing import analyze_results
...
def _stage_writing(project_dir, runtime):
    from openprogram.programs.agentic_functions.research.stages.writing import write_section, gather_context
...
def _stage_review(project_dir, venue, exec_runtime, review_runtime, callback):
    from openprogram.programs.agentic_functions.research.stages.review import review_loop
...
def _stage_submission(project_dir, venue, runtime):
    from openprogram.programs.agentic_functions.research.stages.submission import run_submission_check
