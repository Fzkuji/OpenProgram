...
def _recover_web_use_page(failure: dict, *, backend: str):
    ...
    if "surfaces" not in context:
        return _open_page_error(context)
    try:
        check_cancelled()
        return _start_session_on_opened_page(context=context, owner_id=owner_id, backend=backend, arguments={}, before_dispatch=check_cancelled)
    except BaseException:
        surface_context.release_bindings(context)
        raise
