...
def generate_code(task: str, runtime: Runtime) -> str:
    ...
    body_rules = (
        ...
        "3. Behavioral instructions for the LLM MUST be visible inside\n"
        "   `content`. The docstring is not reliably injected as a system\n"
        "   prompt across providers — some providers (codex CLI) ignore it.\n"
        "   So write the operative instruction (output format, label set,\n"
        "   constraint, etc.) as plain text inside content alongside the\n"
        "   data:\n"
        "\n"
        ...
        "       }])\n"
        "\n"
        "   The docstring still must exist (it documents the function for\n"
        "   humans and the metadata layer), but do not RELY on it to steer\n"
        "   the LLM's output. Steer with content.\n"
        "=== End of hard rules ===\n\n"
    )
