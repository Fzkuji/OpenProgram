...
def _port_available(port: int) -> bool:
    ...
    backend off ``8109`` to a random port for ~60s, which forces a
...
def run_foreground() -> int:
    ...
    # Backend port is fixed (default 8109) so the bundled Next.js
    # frontend's rewrites compile against a stable target.
    import os
    from openprogram.webui import start_web

    port = int(os.environ.get("OPENPROGRAM_BACKEND_PORT", "8109"))
    if not _port_available(port):
        port = _find_free_port()
        print(
            f"[worker] backend port {os.environ.get('OPENPROGRAM_BACKEND_PORT', '8109')} taken; using free port {port}"
        )
