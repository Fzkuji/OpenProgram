...
async def handle_load_session(ws, cmd: dict):
    ...
    if conv:
        ...
        # Use get_branch (called_by traversal) as the primary chain
        # builder. It handles DAG-store sessions (parent_id=None) and
        # ROOT-parented nodes correctly. linear_history (parent_id
        # traversal) is deprecated — parent_id is often unset.
        try:
            chain = _db_load.get_branch(session_id)
        except Exception:
            chain = list(all_msgs)
