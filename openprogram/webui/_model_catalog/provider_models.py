...
``openprogram/providers/<provider>/models.fetched.json`` (gitignored).

Two sources, combined at read time:
  * fetch (provider official /v1/models) — authoritative "which models +
    context", saved per-provider as models.fetched.json (overwritten on each
    Fetch).
...
models.fetched.json yet falls back to models.dev's full list for that
provider.
"""
...
def _resolve_write_path(provider_id: str) -> Path:
    return _provider_dir(provider_id) / "models.fetched.json"


def _resolve_read_path(provider_id: str) -> Path | None:
    p = _provider_dir(provider_id) / "models.fetched.json"
    return p if p.is_file() else None


def save_fetched(provider_id: str, models: list[dict[str, Any]]) -> Path:
    """Overwrite this provider's models.fetched.json with the freshly-fetched list.
