...
_SETUP_HINTS: dict[str, str] = {
    "claude-code": (
        ...
        "Claude Code?** By default it uses whatever `claude auth login` is\n"
        "signed into, so they share one account. To run OpenProgram on a\n"
        "SEPARATE Claude subscription — independent of the terminal login —\n"
        "add it as a Claude account (a browser login that does NOT touch your\n"
        "terminal login) and activate it:\n"
        "\n"
        "$ openprogram providers claude-code accounts add experiment\n"
        "$ openprogram providers claude-code accounts use experiment\n"
        "\n"
        "Or do it below in the `Claude account` control. Empty = follow the\n"
        "terminal login. The terminal Claude Code account is never affected,\n"
        "so you can chat on one account and run OpenProgram on another,\n"
        "switching any time.\n"
        "\n"
