...
_SETUP_HINTS: dict[str, str] = {
    "claude-code": (
        ...
        "needed.\n"
        "\n"
        "**Want OpenProgram on a different Claude account than your terminal\n"
        "Claude Code?** By default Meridian uses whatever `claude auth login`\n"
        "is signed into, so they share one account. To pin OpenProgram to a\n"
        "SEPARATE subscription — independent of the terminal login — add that\n"
        "account as a Meridian profile (a browser login that does NOT touch\n"
        "your terminal login):\n"
        "\n"
        "$ meridian profile add experiment\n"
        "\n"
        "then put its name (`experiment`) in the `Meridian account (profile)`\n"
        "field below — or run `openprogram providers meridian use experiment`.\n"
        "Empty = follow the terminal login. The terminal Claude Code account\n"
        "is never affected, so you can chat on one account and run OpenProgram\n"
        "on another, switching any time by changing that field.\n"
        "\n"
