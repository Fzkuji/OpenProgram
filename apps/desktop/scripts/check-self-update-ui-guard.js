...
const release = guard.acquire(main);
assert.throws(() => guard.acquire(main), /already guarded/);
native.emit("desktop:open-external", { sender: main });
...
request({ webContentsId: 7 }, (value) => { result = value; });
assert.deepEqual(result, { cancel: false });
const second = guard.acquire(main);
release(); // A previous cleanup cannot release a newer capture's lease.
assert.throws(() => handlers.get("updates:download")({ sender: main }), /verification/);
second();
assert.equal(handlers.get("updates:download")({ sender: main }), "downloaded");
assert.throws(() => guard.acquire({ ...main, isDestroyed: () => true }), /unavailable/);
const failedSession = { webRequest: { onBeforeRequest() { throw new Error("hook denied"); } } };
assert.throws(() => guard.acquire({ ...main, session: failedSession }), /hook denied/);
...
const navigationLease = guard.acquire(main);
