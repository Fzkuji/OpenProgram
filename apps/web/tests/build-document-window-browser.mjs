...
await build({ entryPoints: [new URL(process.argv[3] ?? "./document-window-browser-entry.tsx", import.meta.url).pathname], outfile: output, bundle: true, format: "iife", platform: "browser", sourcemap: false, plugins: [cssPlugin], tsconfig: new URL("../tsconfig.json", import.meta.url).pathname, jsx: "automatic", define: { "process.env.NODE_ENV": '"production"' }, loader: { ".svg": "dataurl" } });
