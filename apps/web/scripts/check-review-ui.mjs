...
assert.doesNotMatch(cardCss, /\.turn-files-(?:name|stat|op)\{[^}]*font-(?:family:var\(--font-mono\)|[^}]*var\(--font-mono\))/);
...
assert.match(design, /\.change-summary-stat\{[^}]*var\(--font-sans\)/);
...
assert.match(design, /\.change-row-counts\{[^}]*grid-template-columns:8ch 8ch[^}]*font:12px[^}]*var\(--font-sans\)/);
