...
function UpdateRequests({ update }: { update: SelfUpdate }) {
  ...
  return <details>
    <summary>{text("Request an update action", "请求更新操作")}</summary>
...
export function SelfUpdateCard({ update }: { update: SelfUpdate }) {
  ...
  const iteration = update.iteration;
  return <article className={styles.card} aria-label={text("Self-update", "自更新")} data-update-id={update.update_id}>
    <div className={styles.header}>
      <strong>{text("Self-update", "自更新")} · {text("Attempt", "尝试")} {update.attempt}</strong>
      <span>{text(...PHASES[update.phase])}</span>
    </div>
    <dl className={styles.versions}>
      <div><dt>{text("Target revision", "目标版本")}</dt><dd><code>{update.candidate_revision}</code></dd></div>
      <div><dt>{text("Last verified runtime", "最近已验证运行版本")}</dt><dd>
        {runtime ? <><code>{runtime.candidate_sha}</code><div>{new Date(runtime.verified_at * 1000).toLocaleString()} · PID {runtime.worker_pid} · {runtime.source}</div></> : text("Unknown", "未知")}
      </dd></div>
    </dl>
